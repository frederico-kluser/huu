...
8. [Per-Task Review (`review`)](#per-task-review-review)
9. [Post-Merge Gates (`mergeGate` & `accept`)](#post-merge-gates-mergegate--accept)
10. [Writing Effective Prompts](#writing-effective-prompts)
11. [Model Selection (`modelId`)](#model-selection-modelid)
...
| `mergeGate` | string | No | — | Shell command run in the integration worktree after **every** successful agent-branch merge. Non-zero exit ⇒ the merge commit is rolled back and the branch marked `mergeFailed`. 60-second timeout per merge. See [Post-merge gates](#post-merge-gates-mergegate--accept). |
...
| `review` | ReviewSpec | No | — | Per-task generator → critic loop, run in each agent's OWN worktree BEFORE its branch is eligible for the stage merge. See [Per-Task Review](#per-task-review-review). Work steps only — a `review` on a `CheckStep` is dropped at parse time. |
| `accept` | AcceptSpec | No | — | Shell acceptance gate. Currently runs **after** the step's stage merge, in the integration worktree; a mismatching exit code marks the step's tasks `mergeFailed` and fails the run. See [Post-merge gates](#post-merge-gates-mergegate--accept). |

### ReviewSpec object (per-task critic loop)

Attached to a `WorkStep` as `review`. Only `prompt` is required; every other
field falls back to a constant, so a step that sets nothing else still gets the
whole policy. See [Per-Task Review](#per-task-review-review) for the semantics.

| Field | Type | Required | Default | Constraints |
|---|---|---|---|---|
| `prompt` | string | **Yes** | — | Min 1 char. The critic's briefing — what THIS pipeline wants audited, on top of the generic reviewer prompt huu always sends. |
| `maxRounds` | integer | No | `2` | Range 1–4. Round 1 always runs. Hitting the cap with blocking findings still open **waives** them by default — or **holds** the task for a human when `onBlocked: 'hold'` (see below). |
| `blockOn` | ReviewSeverity[] | No | `["blocker","major"]` | Min 1 entry. Severities that hold the merge. Values: `blocker`, `major`, `minor`, `nit`. An empty array is rejected — it would be a loop that can never converge on anything. |
| `modelId` | string | No | Falls back to global model | The critic's model. Prefer a **different vendor** from the step's `modelId` — see the cross-family note below. |
| `timeoutMs` | integer | No | `min(cardTimeoutMs, 600000)` | Must be > 0. Per-ROUND wall clock for the critic. |
| `maxFindings` | integer | No | `10` | Range 1–50. Hard cap per round, severity-descending; the excess is truncated with a warning. |
| `verifyCommands` | string[] | No | — | Commands the critic MUST run — and paste the output of — **before** writing a single finding. This is your project's real gate (`npm run typecheck && npm test`, or whatever it is). |
| `findingsDir` | string | No | — | Directory (repo-relative, inside the worktree) for per-task finding shards: one `agent-<N>.json` per task, **never** a shared file. Omit and findings are only kept in run state. |
| `onBlocked` | `"waive"` \| `"hold"` | No | `"waive"` | What happens when `maxRounds` is reached with blocking findings still open: record and merge anyway (`waive`), or park the task for a human through the interactive-retry hold (`hold`; headless runs always fall back to `waive`). See [Blocking is a choice: `onBlocked`](#blocking-is-a-choice-onblocked). |

### AcceptSpec object (shell acceptance gate)

Attached to a `WorkStep` as `accept`. See [Post-merge gates](#post-merge-gates-mergegate--accept) for the semantics — including where it actually runs today.

| Field | Type | Required | Default | Constraints |
|---|---|---|---|---|
| `command` | string | **Yes** | — | Shell command executed by the gate. |
| `expectExit` | integer | No | `0` | The exit code treated as success. Any other code — crash, signal, timeout — fails the gate. |
| `cwd` | `"worktree"` \| `"integration"` | No | — | Parsed but **currently ignored**: the gate always executes in the integration worktree. |
...
## Per-Task Review (`review`)

`review` is a **generator → critic loop around ONE task**. Development mode is where it was first used, but it is an ordinary schema field: any work step in any pipeline may declare it.

Compare it to a check node — they solve different problems and do not replace each other:

| | `CheckStep` | `WorkStep.review` |
|---|---|---|
| What it judges | a whole STAGE | ONE task's diff |
| Where it runs | the integration worktree | the WORKER's own worktree |
| When | **after** the branches merged | **before** the branch is eligible for the merge |
| What it can do | route the cursor to another step | send findings back to the SAME agent, in the same session |
| What a "no" costs | re-running the whole stage | one more turn for one agent |

### What happens, round by round

For each task of the step (round 1 always runs, up to `maxRounds`):

1. If the worktree is clean **and** the branch is not ahead of its base, the review is **skipped** — the agent wrote nothing, so there is nothing to criticise.
2. huu commits the round's work on the agent's own branch. That is what lets the critic read a real `git diff <baseRef>..HEAD` range — a dirty worktree hides brand-new files from `git diff` — and it leaves a per-round trail of what each pass changed.
3. A **separate critic agent** is spawned into that same worktree (cwd = the worker's worktree). It is told to run `verifyCommands` and paste the output **first**, then read the diff and the task spec, then turn anything a command actually demonstrated into a finding carrying `proof`, and only then to consider style and design-pattern fit. It emits a fenced JSON block: `{"verdict": …, "findings": [ … ]}`.
4. Findings are recorded on the card, written to `findingsDir` (one shard per task), and surfaced in the log with a `🔍` prefix.
5. **Convergence is mechanical and reads SEVERITY only.** If no finding's severity is listed in `blockOn`, the loop ends. Otherwise the blocking findings go back to the SAME agent (same session, same worktree, one more turn) and the next round begins.

The card's phase moves `RUNNING → REVIEW → FIXING → REVIEW → READY`; there is no separate card for the critic. Review rounds are **intra-step**: they do not consume `maxNodeExecutions` visits.

### The verdict string does not decide — severity does

The critic's own `"verdict"` is logged and displayed, and it is ignored when deciding whether to loop. A critic that writes `"approved"` while listing a `blocker` is still sent back; one that writes `"changes-requested"` over three `nit`s is not. This is the same fixed-enum mechanical-judge discipline check nodes already use, and it is the only formulation a fluent model cannot talk its way around.

A finding with an unknown `severity`/`category`, or with no `file`, is **discarded with a warning** at parse time. An unlocatable claim cannot be acted on, so it must never gate a merge.

### Every failure is forward-default: the work MERGES

This is the part to internalize before designing a pipeline around `review`. **The critic cannot block indefinitely, and it cannot fail a task.** Every degraded path ends with the branch merging:

| What went wrong | What happens |
|---|---|
| Critic threw, timed out, or was abandoned under memory pressure | verdict `unavailable` → counted as **zero blocking** → merge |
| Critic produced no parseable JSON block | verdict `unavailable` → merge |
| Critic returned more than `maxFindings` | truncated severity-descending, with a warning |
| The fix turn threw or timed out | agent aborted, remaining findings **waived**, work so far merges |
| `maxRounds` reached with blockers still open | `onBlocked: 'waive'` (default): **waived** (`reviewWaived`), a `warn` in the log, findings recorded, branch merges. `onBlocked: 'hold'`: the task is parked as a retriable failure and the interactive-retry hold surfaces it for a human (see below); headless runs behave exactly like `'waive'`. |

The alternative — erroring the card — would make `runStageIntegration` exclude it from the merge, turning "90% right with one major finding" into *nothing*. A broken critic must never be able to destroy a good implementation. If you need a hard stop, that is what a `CheckStep` after the step is for.

So do **not** design a pipeline that relies on `review` to guarantee anything. It raises the floor on quality and produces a recorded, machine-readable finding list; it is not a gate. Use a check node when you need routing, and read the waived findings (`findingsDir`, `"waived": true`) when you need to know what got through.

### Blocking is a choice: `onBlocked`

The forward-default table above is deliberate: a broken or unconverged critic must never be able to destroy a good implementation. But "the cap waives" also means a pipeline cannot *insist* on a standard — so `onBlocked` makes the endgame a per-step choice:

- **`'waive'` (default)** — exactly the table above: findings recorded, branch merges, nothing waits.
- **`'hold'`** — reaching the round cap with blocking findings still open parks the task as a **retriable failure** carrying the open findings, and the run's interactive-retry hold (`awaiting_retry` + `retryTask`) surfaces it: **retry** re-runs the task (reviewed again); **abandon** applies the `'waive'` semantics to the preserved branch. The work is never lost either way.
...
- **Wall clock.** Each round is a critic call plus a fix turn, so a reviewed card can take up to `cardTimeout × (1 + maxRounds)`. Budget for it before raising `maxRounds`.
- **Concurrency.** The critic is a *reserved* agent: it is charged to the RAM budget and consumes one pool slot while it runs, and the worker it audits is not preemptible for that window. On a small machine a wide fan-out with `review` roughly halves effective concurrency. That is a real cost, not a bug.
- **Use a cross-family critic.** Point `review.modelId` at a different vendor from the step's `modelId`. A model reviewing its own family's output is the most fragile assumption available here; the measured dominant failure mode of an LLM critic is **spurious blocking of correct code**, not missed bugs.
- **Start narrow.** `blockOn: ["blocker"]` and `maxRounds: 2` first; widen to `["blocker","major"]` once you have watched a few runs.
...
{
  "type": "work",
  "name": "3. Implement the tasks",
  "scope": "memory",
  "filesFrom": ".huu/dev/<session>/epoch-1/api/tasks.json",
  "files": [],
  "modelId": "deepseek/deepseek-v4-pro",
  "prompt": "Implement the task described in $file. …",
  "review": {
    "prompt": "Audit this diff on three axes: correctness against the task spec, this project's design patterns, and its code style. Use the project's own conventions as the standard, not your taste.",
    "modelId": "moonshotai/kimi-k2.6",
    "maxRounds": 2,
    "blockOn": ["blocker", "major"],
    "maxFindings": 10,
    "verifyCommands": ["npm run typecheck", "npm test"],
    "findingsDir": ".huu/dev/<session>/epoch-1/api/review"
  }
}
```
...
## Post-merge gates (`mergeGate` & `accept`)

Two shell gates run on the **merged result**, not on any single agent's diff. Both are deterministic — exit codes, no LLM — and both are opt-in: absent, nothing runs. Compare them with the review loop: the critic judges ONE task's diff with a model; these judge the merged stage with a command.

### `mergeGate` (pipeline level)

A single shell command executed in the integration worktree **after every successful agent-branch merge** — once per branch, on both the deterministic and the LLM-resolved merge paths:

```json
{
  "name": "guarded-pipeline",
  "mergeGate": "npm run lint && npm run typecheck",
  "steps": [ ... ]
}
```

- **Red gate ⇒ the merge is rolled back.** A non-zero exit (or a timeout) runs `git reset --hard HEAD~1` in the integration worktree, undoing ONLY huu's own merge commit — the one merge commit in the system that may be rewound, because it was never pushed and the agent's own branch stays intact. The branch goes back to pending and is marked `mergeFailed`; the remaining branches of the stage keep merging.
- **60-second timeout per merge.** That is the entire budget, so keep the gate to fast static checks (lint, typecheck). A test suite belongs in the critic's `verifyCommands`, not here.
- **A red gate fails the stage.** The branch's work is preserved on the agent's own branch for inspection, and the run either holds for a human retry (interactive-retry channel wired) or stops with the gate's output in the error.

### `accept` (work-step level)

```json
{
  "type": "work",
  "name": "2. Generate the report",
  "prompt": "Write the audit report to REPORT.md.",
  "files": [],
  "scope": "project",
  "accept": { "command": "test -s REPORT.md", "expectExit": 0 }
}
```

- **Where it runs today:** **after the step's stage merge**, in the integration worktree, over the merged tree (120-second timeout). The schema also parses a `cwd` field (`'worktree' | 'integration'`), but the executor currently ignores it — the gate always runs in the integration worktree.
- **A mismatch fails the run.** If the exit code differs from `expectExit` (default `0`; crashes and signals count as failures too), every task of the step is marked `mergeFailed` and the run stops with the gate's output in the error. There is no retry loop inside the gate: it is a hard stop, so use it for invariants that MUST hold before the pipeline continues (`test -s report.md`, `grep -q …`, a schema-validation script).
- **`mergeGate` vs `accept`:** `mergeGate` fires per branch merge and reverts that one merge; `accept` fires once per step after the whole stage merged and stops the run.
...
- [ ] `mergeGate`, if set, is a fast command (lint/typecheck) — it runs per merge under a 60-second timeout
- [ ] `accept.expectExit`, if set, matches the exit code the gate command produces on success
