...
On first run, huu materializes six framework-agnostic starter pipelines
...
**Strict report-only contract for the five audits.** They write ONLY to
`.huu/audits/<topic>.md` and `.huu/audits/<topic>-faq.json`. They never
modify your README, `package.json`, lockfiles, or any production source.
Tools that need to be invoked (semgrep, jscpd, gitleaks, lighthouse-ci,
…) are run ephemerally via `npx --yes`, `pipx run`, or vendored
binaries under `$HOME/.huu/bin/` — never added to your project's
manifests. The only pipeline that touches production files is
`huu Test Suite`, which is a setup pipeline (writes `huu-tests.md` and
a tests badge in README — that's intentional).
