...
export function PipelineAssistant({
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.ai}>{t('tui.assistant.title')}</Text>
          <Text dimColor>{t('tui.assistant.model_purpose')}</Text>
          <Text dimColor>
            {t('tui.assistant.model_advice')}{' '}
            <Text color={theme.ai}>deepseek/deepseek-v4-pro</Text> · moonshotai/kimi-k2.6 · openai/gpt-5.4 · anthropic/claude-opus-4.6.{' '}
            {t('tui.assistant.model_default', { model: DEFAULT_ASSISTANT_MODEL })}
          </Text>
        </Box>
        <Box marginTop={1}>
  ...
  if (stage.kind === 'intent') {
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.ai}>{t('tui.assistant.title')}</Text>
          <Text dimColor>{t('tui.assistant.model', { model: modelId })}</Text>

          <Box marginTop={1} flexDirection="column">
            <Text>{t('tui.assistant.intent_question')}</Text>
            <Text dimColor>{t('tui.assistant.intent_hint')}</Text>
          </Box>
          ...
          <Box marginTop={1}>
            <Text dimColor>
              <Text bold>ENTER</Text> {t('tui.assistant.hint_start')} · <Text bold>ESC</Text>{' '}
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.ai}>{t('tui.assistant.title')}</Text>
          <Text dimColor>
            {t('tui.assistant.model_turn', { model: modelId, turn: turnsAsked + 1 })}
          </Text>
          ...
          {history.length > 0 && (
            <Box marginTop={1} flexDirection="column">
              {history.slice(-4).map((entry, i) => (
                <Box key={i}>
                  <Text color={entry.role === 'user' ? 'cyan' : 'yellow'} bold>
                    {entry.role === 'user'
                      ? `${t('tui.assistant.speaker_you')} `
                      : `${t('tui.assistant.speaker_assistant')} `}
                  </Text>
          ...
          <Box marginTop={1}>
            <Spinner label={t('tui.assistant.thinking')} color={theme.ai} />
          </Box>

          <Box marginTop={1}>
            <Text dimColor>
              <Text bold>ESC</Text> {t('common.action.cancel')}
            </Text>
          </Box>
        </Box>
      </Box>
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.ai}>{t('tui.assistant.title')}</Text>
          <Text dimColor>
            {t('tui.assistant.model_turn', { model: modelId, turn: turnsAsked })}
          </Text>
          ...
          <Box marginTop={1}>
            <Text dimColor>
              <Text bold>1-{stage.turn.options.length}</Text> {t('common.action.select')} ·{' '}
              <Text bold>ESC</Text> {t('common.action.cancel')}
            </Text>
          </Box>
        </Box>
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          ...
          <Box marginTop={1} flexDirection="column">
            <Text bold>{stage.turn.question}</Text>
            <Text dimColor>{t('tui.assistant.free_text_hint')}</Text>
          </Box>
          <Box marginTop={1}>
            <Text color="cyan">› </Text>
            <TextInput
          ...
          <Box marginTop={1}>
            <Text dimColor>
              <Text bold>ENTER</Text> {t('tui.assistant.hint_send')} · <Text bold>ESC</Text>{' '}
  ...
  if (stage.kind === 'architect') {
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.ai}>{t('tui.assistant.architect_title')}</Text>
          <Text dimColor>{t('tui.assistant.architect_subtitle', { model: modelId })}</Text>
          ...
          <Box marginTop={1} flexDirection="column">
            ...
            {stage.phases.length === 0 && (
              <Spinner label={t('tui.assistant.architect_starting')} color={theme.ai} />
            )}
          </Box>
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor="yellow" paddingX={1} flexDirection="column" width="100%">
          <Text bold color="yellow">{t('tui.assistant.discard_title')}</Text>
          <Text dimColor>{t('tui.assistant.discard_body')}</Text>
          <Box marginTop={1}>
            <Text>
              <Text bold color="cyan">[Y]</Text> {t('tui.assistant.discard_yes')}  ·{' '}
              <Text bold color="cyan">[N]</Text> {t('tui.assistant.discard_no')}
            </Text>
          </Box>
        </Box>
      </Box>
  ...
  return (
    <Box flexDirection="column" width="100%">
      <Box borderStyle="round" borderColor="red" paddingX={1} flexDirection="column" width="100%">
        <Text bold color="red">{t('tui.assistant.error_title')}</Text>
        <Box marginTop={1}><Text>{stage.message}</Text></Box>
        <Box marginTop={1}><Text dimColor>{t('tui.assistant.error_hint')}</Text></Box>
      </Box>
    </Box>
  );
}
