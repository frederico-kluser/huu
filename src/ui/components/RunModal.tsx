...
function timelineEntries(
  ...
  return [
    { key: 'created', label: t('tui.modal.tl_created'), status: 'done' },
    {
      key: 'running',
      label: t('tui.modal.tl_running'),
      status: isError ? 'error' : reachedDone ? 'done' : reachedRunning ? 'active' : 'pending',
    },
    {
      key: 'final',
      label: isError
        ? t('tui.modal.tl_failed')
        : reachedDone
          ? agent.commitSha
            ? t('tui.modal.tl_merged')
            : t('tui.modal.tl_done')
          : t('tui.modal.tl_pending'),
...
export function RunModal({ agent, stepPrompt, onClose }: Props): React.JSX.Element {
  ...
  const lines: BodyLine[] = useMemo(() => {
    ...
    sectionHeader(t('tui.modal.sec_status'), 'status');
    out.push({
      key: 'status-row',
      node: (
        <Box>
          <Text bold color={stateColor(agent.state)}>● {agent.state}</Text>
          <Text dimColor>{`  · ${t('tui.modal.phase')} `}</Text>
          <Text>{agent.phase}</Text>
          <Text dimColor>  · </Text>
          <Text>{elapsed}</Text>
          <Text dimColor>{`  · ${t('tui.modal.tokens')} `}</Text>
          <Text>{tokenInfo}</Text>
          {agent.cost > 0 && (
            <React.Fragment>
              <Text dimColor>{`  · ${t('tui.modal.cost')} `}</Text>
      ...
      out.push({
        ...
        node: (
          <Text wrap="truncate-end">
            <Text dimColor>{`  ${t('tui.modal.on')} `}</Text>
            {agent.currentFile}
          </Text>
        ),
      });
    ...
    blank('1');
    sectionHeader(t('tui.modal.sec_timeline'), 'timeline');
    ...
    blank('2');
    sectionHeader(t('tui.modal.sec_git'), 'git');
    // Labels are padded to a fixed column so the values stay aligned in every
    // locale — the padding lives here, not inside the translated string.
    const gitLabel = (key: string): string => `  ${key.padEnd(9)}`;
    out.push({
      key: 'git-branch',
      node: (
        <Text wrap="truncate-end">
          <Text dimColor>{gitLabel(t('tui.modal.git_branch'))}</Text>
          {agent.branchName ?? <Text dimColor>{t('common.pending')}</Text>}
        </Text>
      ),
    });
    out.push({
      ...
      node: (
        <Text wrap="truncate-end">
          <Text dimColor>{gitLabel(t('tui.modal.git_worktree'))}</Text>
          {agent.worktreePath ?? <Text dimColor>{t('common.pending')}</Text>}
        </Text>
      ),
    });
    out.push({
      ...
      node: (
        <Text wrap="truncate-end">
          <Text dimColor>{gitLabel(t('tui.modal.git_commit'))}</Text>
          {agent.commitSha ?? <Text dimColor>{t('common.none')}</Text>}
        </Text>
      ),
    });
    out.push({
      ...
      node: (
        <Text wrap="truncate-end">
          <Text dimColor>{gitLabel(t('tui.modal.git_stage'))}</Text>
    ...
    blank('3');
    sectionHeader(t('tui.modal.sec_prompt'), 'prompt');
    ...
    if (agent.filesModified.length > 0) {
      blank('4');
      sectionHeader(t('tui.modal.sec_files', { count: agent.filesModified.length }), 'files');
      const FILE_LIMIT = 50;
      ...
      if (agent.filesModified.length > FILE_LIMIT) {
        out.push({
          ...
          node: (
            <Text dimColor>
              {`  ${t('tui.modal.more_files', { count: agent.filesModified.length - FILE_LIMIT })}`}
            </Text>
          ),
        });
      }
    ...
    if (agent.error) {
      blank('5');
      sectionHeader(t('tui.modal.sec_error'), 'error');
    ...
    blank('6');
    sectionHeader(t('tui.modal.sec_logs', { count: agent.logs.length }), 'logs');
    if (agent.logs.length === 0) {
      out.push({
        key: 'logs-empty',
        node: <Text dimColor>{`  ${t('tui.modal.logs_waiting')}`}</Text>,
      });
    } else {
  ...
  const scrollLabel =
    totalLines <= bodyRows
      ? t('tui.modal.scroll_all')
      : `${scrollTop + 1}–${lastVisible} / ${totalLines}`;
  const followLabel =
    followTailRef.current && totalLines > bodyRows ? ` · ${t('tui.modal.follow')}` : '';
    ...
    <Box flexDirection="column" width="100%">
      ...
      >
        <Box justifyContent="space-between" width="100%">
          <Text wrap="truncate-end">
            <Text bold color="cyan">{t('tui.modal.agent_title', { id: agent.agentId })}</Text>
            <Text dimColor>  ·  </Text>
          ...
          <Text>
            ...
            <Text color={isRunning ? 'cyan' : 'gray'}>
              {isRunning ? `● ${t('tui.modal.live')}` : `○ ${t('tui.modal.stopped')}`}
            </Text>
          </Text>
        </Box>
        ...
        <Box width="100%">
          <Text dimColor wrap="truncate-end">
            <Text bold>↑↓</Text> {t('tui.modal.hint_scroll')} · <Text bold>PgUp/PgDn</Text>{' '}
            {t('tui.modal.hint_page')} · <Text bold>g/G</Text> {t('tui.modal.hint_top_bottom')} ·{' '}
            <Text bold>ESC</Text> {t('common.action.close')}
          </Text>
        </Box>
      </Box>
    </Box>
