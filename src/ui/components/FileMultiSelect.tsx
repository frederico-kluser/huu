...
export function FileMultiSelect({
  ...
  const runSmartSuggest = (overrideModelId?: string): void => {
    ...
    if (!currentStep.prompt.trim()) {
      setSmart({
        kind: 'error',
        message: t('tui.files.err_no_prompt'),
      });
      return;
    }
    const abort = new AbortController();
    setSmart({ kind: 'loading', abort, logs: [t('tui.files.smart_init')] });
  ...
  const regexInfo = useMemo(() => {
    ...
    try {
      ...
      return { regex: null, error: e instanceof Error ? e.message : t('tui.files.invalid_regex') };
    }
  }, [regexPattern]);
  ...
  const handleRegexChange = (next: string) => {
    ...
    try {
      ...
      setRegexError(e instanceof Error ? e.message : t('tui.files.invalid_regex'));
    }
  };
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.info} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.info}>{t('tui.files.prev_title')}</Text>
          <Text dimColor>{t('tui.files.prev_subtitle')}</Text>
            ...
            {availablePreviousSteps.map((s, i) => {
              ...
              return (
                <Text key={s.index} color={isCursor ? 'cyan' : undefined} bold={isCursor}>
                  {isCursor ? '> ' : '  '}
                  #{s.index + 1}  {s.name || <Text dimColor italic>{t('common.unnamed')}</Text>}
                  <Text dimColor>  ·  </Text>
                  <Text color="green">
                    {s.files.length === 1
                      ? t('tui.files.file_count_one', { count: s.files.length })
                      : t('tui.files.file_count_other', { count: s.files.length })}
                  </Text>
                </Text>
              );
            })}
          ...
          <Box marginTop={1}>
            <Text dimColor>
              ...
              {t('common.action.copy')} · <Text bold>ESC</Text> {t('common.action.cancel')}
            </Text>
          </Box>
        </Box>
      </Box>
    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box borderStyle="round" borderColor={theme.ai} paddingX={1} flexDirection="column" width="100%">
          <Text bold color={theme.ai}>{t('tui.files.smart_title')}</Text>
          <Text dimColor>
            {t('tui.files.step_label', {
              index: currentStepIndex + 1,
              name: currentStep.name || t('common.unnamed'),
            })}
          </Text>

          <Box marginTop={1}>
            <Spinner label={t('tui.files.model_label', { model: smartModelLabel })} color={theme.ai} />
          </Box>
          ...
          <Box marginTop={1}>
            <Text dimColor>
              <Text bold>ESC</Text> {t('common.action.abort')}
            </Text>
          </Box>
        </Box>
      </Box>
  ...
  return (
    <Box flexDirection="column" width="100%">
      <Box borderStyle="round" borderColor={borderColor} paddingX={1} flexDirection="column" width="100%">
        <Box>
          <Text bold color="cyan">{t('tui.files.title')}</Text>
          <Text dimColor>  ·  </Text>
          {totalSelected === 0 ? (
            <Text color="yellow">{t('tui.files.none_selected')}</Text>
          ) : (
            <Text color="green">{t('tui.files.selected_count', { count: totalSelected })}</Text>
          )}
        </Box>

        {smart.kind === 'error' && (
          <Box marginTop={1}>
            <Text color={theme.error}>{t('tui.files.smart_failed', { message: smart.message })}</Text>
            <Text dimColor>{`   ${t('tui.files.retry_hint')}`}</Text>
          </Box>
        )}
        {smart.kind === 'success' && (
          <Box marginTop={1}>
            <Text color={theme.ai}>
              {smart.suggestedSet.size === 1
                ? t('tui.files.smart_suggested_one', { count: smart.suggestedSet.size })
                : t('tui.files.smart_suggested_other', { count: smart.suggestedSet.size })}
            </Text>
            {smart.ignoredCount > 0 && (
              <Text dimColor>{`   ${t('tui.files.ignored', { count: smart.ignoredCount })}`}</Text>
            )}
          </Box>
        )}

        <Box marginTop={1}>
          <Text>{t('tui.files.filter')}</Text>
          {filterMode ? (
            <TextInput value={filter} onChange={setFilter} onSubmit={() => setFilterMode(false)} />
          ) : (
            <>
              <Text>{filter || <Text dimColor>{t('common.none')}</Text>}</Text>
              <Text dimColor>{`   ${t('tui.files.press_slash')}`}</Text>
            </>
          )}
        </Box>

        <Box>
          <Text>{t('tui.files.regex')}</Text>
          {regexMode ? (
            <>
              ...
              {regexError ? (
                ...
                <Text color="green">
                  {'  · '}
                  {regexMatchCount === 1
                    ? t('tui.files.match_count_one', { count: regexMatchCount })
                    : t('tui.files.match_count_other', { count: regexMatchCount })}
                </Text>
              ) : (
                <Text dimColor>{`  · ${t('tui.files.regex_hint')}`}</Text>
              )}
            </>
          ) : (
            <>
              <Text>{regexPattern || <Text dimColor>{t('common.none')}</Text>}</Text>
              <Text dimColor>{`   ${t('tui.files.press_r')}`}</Text>
            </>
          )}
        </Box>

        <Box flexDirection="column" marginTop={1}>
          {visibleSlice.length === 0 && <Text dimColor>{t('tui.files.no_match')}</Text>}
        ...
        <Box marginTop={1} flexDirection="column">
          <Text dimColor>
            <Text bold>↑↓</Text> {t('common.action.navigate')} · <Text bold>←/→</Text>{' '}
            {t('tui.files.hint_collapse')} · <Text bold>SPACE</Text> {t('common.action.toggle')} ·{' '}
            <Text bold>E</Text> {t('tui.files.hint_expand_all')}
          </Text>
          <Text dimColor>
            <Text bold>A</Text> {t('tui.files.hint_select_all')} · <Text bold>C</Text>{' '}
            {t('common.action.clear')} · <Text bold>/</Text> {t('tui.files.hint_filter')} ·{' '}
            <Text bold>r</Text> {t('tui.files.hint_regex')} ·{' '}
            <Text color={theme.ai} bold>S</Text>{' '}
            <Text color={theme.ai}>{t('tui.files.hint_ai')}</Text>
            {availablePreviousSteps.length > 0 && (
              <> · <Text bold>P</Text> {t('tui.files.hint_copy_prev')}</>
            )}
