    ...
    <Box flexDirection="column" width="100%">
      <Box borderStyle="round" borderColor="cyan" paddingX={1} flexDirection="column" width="100%">
        ...
        <Box marginTop={1} flexDirection="column">
          <Text>{t('tui.paste.instructions')}</Text>
          <Text dimColor>{t('tui.paste.autodetect')}</Text>
        </Box>
        ...
        <Box marginTop={1}>
          {status.kind === 'waiting' && (
            <Text>
              {charCount === 0 ? (
                <Text dimColor>{t('tui.paste.waiting')}</Text>
              ) : (
                <Text dimColor>{t('tui.paste.buffering', { count: charCount })}</Text>
              )}
            </Text>
          )}
          {status.kind === 'success' && (
            <Text color="green">{t('tui.paste.imported', { name: status.name })}</Text>
          )}
          {status.kind === 'error' && (
        ...
        {status.kind === 'error' && (
          <Box marginTop={1}>
            <Text dimColor>
              <Text bold>Ctrl+R</Text> {t('tui.paste.reset_hint')}
            </Text>
          </Box>
        )}
