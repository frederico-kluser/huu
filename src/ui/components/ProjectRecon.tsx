    ...
    return (
      <Box flexDirection="column" width="100%">
        <Box
          ...
          width="100%"
        >
          <Text bold color={theme.ai}>{t('tui.recon.title')}</Text>
          <Text dimColor>{t('tui.recon.model', { model: renderedModel })}</Text>
          <Box marginTop={1}>
            <Spinner color={theme.ai} label={t('tui.recon.selecting')} />
          </Box>
          {selectorError && (
            <Box marginTop={1}>
              <Text color="red">
                {t('tui.recon.selector_failed', { message: selectorError.slice(0, 80) })}
              </Text>
            </Box>
          )}
          <Box marginTop={1}>
  ...
  return (
    <Box flexDirection="column" width="100%">
      ...
      >
        <Text bold color={theme.ai}>{t('tui.recon.title')}</Text>
        <Text dimColor>
          {t('tui.recon.progress', { total, model: renderedModel, settled })}
        </Text>
    ...
    <Box flexDirection="column" marginBottom={state.status === 'done' ? 1 : 0}>
      <Box>
        {state.status === 'done' ? (
          ...
          <Spinner color={theme.ai} />
        )}
        <Text> </Text>
        <Text bold>{item.label}</Text>
        {item.source === 'custom' && (
          <Text dimColor>{`  (${t('tui.recon.custom')})`}</Text>
        )}
