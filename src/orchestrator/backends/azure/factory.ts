...
import { pickThinkingLevel, pickToolAllowlist } from '../pi/factory.js';
...
export const azureAgentFactory: AgentFactory = async (
  ...
  const { session } = await createAgentSession({
    ...
    cwd,
    // Same role-based tool allowlist as the OpenRouter path — both factories
    // compose the SAME pi session, so a read-only role must be read-only on
    // whichever provider the run happens to use.
    ...(() => {
      const tools = pickToolAllowlist(task.readOnly === true, piEnv.hermetic);
      return tools ? { tools } : {};
    })(),
  });
