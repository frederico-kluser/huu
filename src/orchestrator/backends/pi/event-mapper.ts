...
export function translatePiEvent(
  ...
  switch (ev.type) {
    ...
    // `compaction_start` — NOT `auto_compaction_start`.
    //
    // huu listened for the latter, and pi 0.73.x emits neither that name nor
    // anything close: `AgentSessionEvent` declares `compaction_start` with
    // `reason: "manual" | "threshold" | "overflow"`. The old case was dead
    // code, so a card whose context was thrashing produced no signal at all —
    // not even the warning line the comment promised. The stale name is
    // accepted alongside the real one so an older pi still reports.
