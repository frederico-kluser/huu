...
describe('resolveWorktreeSkillPaths', () => {
  it('returns only the skill roots that exist, in preference order', () => {
    const dir = mkdtempSync(join(tmpdir(), 'huu-skills-'));
    mkdirSync(join(dir, '.agents', 'skills'), { recursive: true });
    mkdirSync(join(dir, '.opencode', 'skills'), { recursive: true });
    expect(resolveWorktreeSkillPaths(dir)).toEqual([
      join(dir, '.agents/skills'),
      join(dir, '.opencode/skills'),
    ]);
  });

  it('is silent for a project with no skills', () => {
    // pi records a DIAGNOSTIC ERROR for a skill path that does not exist, so
    // handing it a speculative path would make every skill-less project noisy.
    const dir = mkdtempSync(join(tmpdir(), 'huu-noskills-'));
    expect(resolveWorktreeSkillPaths(dir)).toEqual([]);
  });
  ...
  it('never walks above the worktree', () => {
    ...
    expect(resolveWorktreeSkillPaths(child)).toEqual([]);
  });
});
