...
/**
 * Skill roots huu recognizes, in the order `knowledge-detect.ts` prefers them.
 * Relative to the agent WORKTREE, never to `$HOME` and never to an ancestor —
 * the same scoping rule the repo-context loader follows.
 */
const SKILL_ROOTS = ['.agents/skills', '.claude/skills', '.opencode/skills'];
 ...
 *
 * Why it is worth loading at all: pi renders every loaded skill into the system
 * prompt as `<available_skills>` — name, description and file path only — with
 * the instruction to read the body with the `read` tool when the task matches.
 * That is progressive disclosure of exactly the project's own conventions, at
 * the cost of a few hundred tokens of descriptions. It also replaces something
 * that was silently broken: dev mode prefixed `/skill:project-router` onto every
 * prompt, which pi can only expand when it is the FIRST characters of the
 * message (`agent-session.js` `_expandSkillCommand`) — and huu's role header
 * always precedes it.
 *
 * Only EXISTING directories are returned: pi records a diagnostic error for a
 * skill path that does not exist, and a project with no skills must stay silent.
 */
export function resolveWorktreeSkillPaths(cwd: string): string[] {
  const out: string[] = [];
  for (const rel of SKILL_ROOTS) {
    ...
    try {
      ...
      /* unreadable — skip, never block a spawn */
    }
  }
  return out;
}
...
export interface BuildPiSessionEnvironmentOptions {
  ...
  /**
   * Default true: load the worktree's OWN skill roots (`.agents/skills`,
   * `.claude/skills`, `.opencode/skills`) so their names + descriptions reach
   * the system prompt and the agent can read a body on demand. Host-global
   * discovery stays off either way. Set false to reproduce the pre-0.x
   * behavior (no skills at all).
   */
  includeWorktreeSkills?: boolean;
...
export async function buildPiSessionEnvironment(
  ...
  const skillPaths =
    opts.includeWorktreeSkills !== false ? resolveWorktreeSkillPaths(opts.cwd) : [];
