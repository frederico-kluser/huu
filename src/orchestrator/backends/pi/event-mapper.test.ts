...
describe('compaction events', () => {
  it('maps pi 0.73.x `compaction_start` to a typed compaction event', () => {
    // huu listened for `auto_compaction_start`, which pi does not emit — the
    // branch was dead, so a card whose context was thrashing produced no signal
    // at all, not even the warning line its comment promised.
    const seen: AgentEvent[] = [];
    translatePiEvent({ type: 'compaction_start', reason: 'threshold' }, (e) => seen.push(e));
    expect(seen).toEqual([{ type: 'compaction', reason: 'threshold' }]);
  });

  it('still accepts the stale name, so an older pi keeps reporting', () => {
    const seen: AgentEvent[] = [];
    translatePiEvent({ type: 'auto_compaction_start', reason: 'overflow' }, (e) => seen.push(e));
    expect(seen).toEqual([{ type: 'compaction', reason: 'overflow' }]);
  });

  it('never throws on a reason the SDK did not send', () => {
    const seen: AgentEvent[] = [];
    translatePiEvent({ type: 'compaction_start' }, (e) => seen.push(e));
    expect(seen).toEqual([{ type: 'compaction', reason: 'unknown' }]);
  });
});
