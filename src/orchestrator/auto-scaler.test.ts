...
describe('AutoScaler', () => {
  ...
  describe('shouldSpawn — stop threshold (OR logic)', () => {
    // The stop threshold gates GROWTH, so each case needs one agent already
    // live — at zero the floor-of-one below deliberately overrides it.
  ...
  describe('shouldSpawn — floor of one (degrade to sequential, never to zero)', () => {
    // Regression: the machine-state gates below are all satisfiable by RAM huu
    // does not own (browser/IDE/loaded box). With zero live agents they used to
    // mean zero agents FOREVER — the pool spun, cost stayed $0, the run never
    // finished. Symptom in the suite: real-git integration tests timing out
    // only on loaded boxes.
    it('admits ONE agent above the stop threshold when nothing is live', () => {
      const scaler = createScaler(makeMetrics({ cpuPercent: 95, ramPercent: 95 }));
      scaler.start();
      expect(scaler.shouldSpawn()).toBe(true);
      // …and closes again the moment that one agent exists.
      scaler.notifyAgentSpawned();
      expect(scaler.shouldSpawn()).toBe(false);
      scaler.stop();
    });

    it('admits ONE agent through the PSI freeze when nothing is live', () => {
    ...
    it('admits ONE agent in manual mode at the destroy threshold when nothing is live', () => {
    ...
    it('admits ONE agent in greedy mode with zero byte headroom when nothing is live', () => {
      // 14 GiB used of 16 → the dial leaves no headroom, so budgetAdditional()
      // is 0: the greedy gate would otherwise never open again.
      const scaler = createScaler(
    ...
    it('does NOT floor through COOLDOWN — our own hold is time-bounded, not a deadlock', () => {
      const scaler = createScaler(makeMetrics({ cpuPercent: 50, ramPercent: 50 }));
      scaler.start();
      scaler.notifyAgentSpawned();
      scaler.notifyAgentDestroyed(); // → zero live agents AND cooldown
  ...
  describe('PSI admission brake (shouldSpawn)', () => {
    it('freezes admission at/above the cut band (2x setpoint = 1.0, controller on)', () => {
      ...
      scaler.start();
      scaler.notifyAgentSpawned(); // freeze GROWTH; at zero the floor-of-one wins
      expect(scaler.shouldSpawn()).toBe(false);
      scaler.stop();
    });
  ...
  describe('manual mode (guard-only)', () => {
    ...
    it('blocks spawning at the destroy threshold', () => {
      ...
      scaler.setMode('manual');
      scaler.start();
      scaler.notifyAgentSpawned(); // spawning would kill someone else; at zero there is no one
      expect(scaler.shouldSpawn()).toBe(false);
      scaler.stop();
    });
  ...
  describe('greedy mode (MAX) — flood to the limit', () => {
    ...
    it('BUDGET-greedy: stops spawning once used bytes cross the dial, even below 95%', () => {
      ...
      scaler.setMode('greedy');
      scaler.start();
      scaler.notifyAgentSpawned(); // the dial caps GROWTH; at zero the floor-of-one wins
      expect(scaler.shouldSpawn()).toBe(false);
      scaler.stop();
    });
...
describe('host-availability clamp (the container is blind to other host processes)', () => {
  const GiB = 1024 ** 3;
  beforeEach(() => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
  });
  ...
  function containerMetrics(hostAvailGiB: number): SystemMetrics {
    // A 14 GiB container (--memory = 16 GiB host − 2 GiB reserve), lightly
    // used INSIDE, while the HOST has only hostAvailGiB left.
    return makeMetrics({
      ramTotalBytes: 14 * GiB,
      ramUsedBytes: 2 * GiB,
      containerAware: true,
      hostMemTotalBytes: 16 * GiB,
      hostMemAvailableBytes: hostAvailGiB * GiB,
    });
  }

  it('binds admission to host availability: cgroup headroom is ignored when the host is tight', () => {
    // Budget side: 85% × 14 GiB − 2 GiB used ≈ 9.9 GiB of headroom.
    // Host side: 3 GiB avail − 2 GiB reserve (of the 16 GiB host) = 1 GiB.
    const scaler = new AutoScaler({ resourceMonitor: () => containerMetrics(3), budgetPercent: 85 });
    scaler.start();
    expect(scaler.headroomBytesRemaining()).toBe(1 * GiB);
    // 1 GiB < the 1536 MiB pessimistic charge → zero additional agents.
    scaler.notifyTaskQueued(10);
    expect(scaler.targetConcurrency()).toBe(1); // floor-of-one, not 6+
    expect(scaler.getStatus().hostClampActive).toBe(true);
    expect(scaler.getStatus().hostAvailableMb).toBe(3 * 1024);
    scaler.stop();
  });

  it('greedy (MAX) also stops at the host clamp, not only at the dial', () => {
    const scaler = new AutoScaler({ resourceMonitor: () => containerMetrics(2), budgetPercent: 85 });
    scaler.setMode('greedy');
    scaler.start();
    // Host headroom = 2 − 2 = 0 → budgetAdditional 0 → greedy gate closes.
    // One agent must already be live, else the floor-of-one admits it.
    scaler.notifyAgentSpawned();
    expect(scaler.shouldSpawn()).toBe(false);
    scaler.stop();
  });

  it('does NOT clamp when the metrics are host-scoped (used already includes other processes)', () => {
    const scaler = new AutoScaler({
      resourceMonitor: () =>
        makeMetrics({
          containerAware: false,
          hostMemTotalBytes: 16 * GiB,
          hostMemAvailableBytes: 3 * GiB,
        }),
      budgetPercent: 85,
    });
    scaler.start();
    // Budget side only: 85% × 16 GiB − 8 GiB used = 5.6 GiB.
    expect(scaler.headroomBytesRemaining()).toBeCloseTo(5.6 * GiB, -6);
    expect(scaler.getStatus().hostClampActive).toBe(false);
    scaler.stop();
  });

  it('HUU_NO_HOST_CLAMP=1 disables the clamp (dedicated-host escape)', () => {
    const saved = process.env.HUU_NO_HOST_CLAMP;
    process.env.HUU_NO_HOST_CLAMP = '1';
    try {
      const scaler = new AutoScaler({ resourceMonitor: () => containerMetrics(3), budgetPercent: 85 });
      scaler.start();
      // Budget side: 85% × 14 − 2 used = 9.9 GiB — host ignored.
      expect(scaler.headroomBytesRemaining()).toBeCloseTo(9.9 * GiB, -6);
      expect(scaler.getStatus().hostClampActive).toBe(false);
      scaler.stop();
    } finally {
      if (saved === undefined) delete process.env.HUU_NO_HOST_CLAMP;
      else process.env.HUU_NO_HOST_CLAMP = saved;
    }
  });

  it('smoothing is asymmetric: a host-avail DROP binds instantly, recovery is damped', () => {
    let current = containerMetrics(8);
    const scaler = new AutoScaler({ resourceMonitor: () => current, budgetPercent: 85 });
    scaler.start(); // first poll seeds the smoothed value at 8 GiB
    expect(scaler.getStatus().hostAvailableMb).toBe(8 * 1024);

    current = containerMetrics(3); // a browser opened on the host
    vi.advanceTimersByTime(1_100); // next slow tick
    expect(scaler.getStatus().hostAvailableMb).toBe(3 * 1024); // instant

    current = containerMetrics(8); // the browser closed again
    vi.advanceTimersByTime(1_100);
    // Damped recovery: 3 + 0.2 × (8 − 3) = 4 GiB, NOT straight back to 8.
    expect(scaler.getStatus().hostAvailableMb).toBe(4 * 1024);
    scaler.stop();
  });

  it('containerAware budget skips the double reserve at high dials (E fix)', () => {
    const scaler = new AutoScaler({
      resourceMonitor: () =>
        makeMetrics({ ramTotalBytes: 14 * GiB, containerAware: true, hostMemAvailableBytes: 14 * GiB, hostMemTotalBytes: 16 * GiB }),
      budgetPercent: 95,
    });
    // 95% of 14 GiB = 13.3 GiB — NOT capped at 14 − reserve(14) = 12 GiB.
    expect(scaler.budgetBytes()).toBeCloseTo(14 * GiB * 0.95, -6);
  });
});
