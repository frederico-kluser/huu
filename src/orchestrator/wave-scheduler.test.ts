...
describe('computeWave', () => {
  ...
  it('a ready check is a barrier only when it is the first ready step', () => {
    // Check as FIRST — singleton wave.
    const steps = [
      check('gate', []),
      work('a', { dependsOn: [] }),
      work('b', { dependsOn: [] }),
    ];
    const wave1 = computeWave(steps, new Set(), new Set(['gate', 'a', 'b']));
    expect(wave1.map((s) => s.name)).toEqual(['gate']);
  });

  it('work steps before a check are NOT preempted (M3-04: wave-scheduler check preempta)', () => {
    // work steps earlier in the array MUST run alongside the check.
    const steps = [
      work('a', { dependsOn: [] }),
      work('b', { dependsOn: [] }),
      check('gate', []),
    ...
    expect(wave.map((s) => s.name)).toEqual(['a', 'b']);
  });
