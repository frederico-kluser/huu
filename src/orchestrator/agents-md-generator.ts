...
- **NEVER run \`git push\`** — the orchestrator manages remote synchronization. Pushing from an agent worktree will break the pipeline.
- Focus only on reading and modifying code.
`
    : '';

  const portContext = ports ? formatPortGuidanceForPrompt(ports, shimAvailable) : '';
...
5. **NEVER run \`git push\`** — the orchestrator manages remote synchronization. Pushing from an agent will break the pipeline.
6. Preserve existing public APIs unless the task explicitly requires changes.
7. Maintain or improve test coverage if tests exist.
8. Follow the existing code style and conventions of the project.
...
4. Do NOT run git commands — the orchestrator manages all Git operations.
5. **NEVER run \`git push\`** — the orchestrator manages remote synchronization. Pushing from an agent will break the pipeline.
6. Preserve existing public APIs unless the refactoring explicitly requires changes.
7. Maintain or improve test coverage if tests exist.
8. Follow the existing code style and conventions of the project.
9. After completing each file, briefly note what was changed and why.
