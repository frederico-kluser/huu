...
export function computeWave(
  ...
  // M3-04: a ready check in any position must NOT preempt the entire wave.
  // Work steps that come BEFORE it in array order must still run. The check
  // itself is still a barrier — it runs as a singleton in the next wave.
  // Previously ANY ready check preempted the whole wave, silently skipping
  // work steps earlier in the array.
  const firstCheckIdx = ready.findIndex((s) => isCheckStep(s));
  if (firstCheckIdx === 0) return [ready[0]!];
  if (firstCheckIdx > 0) return ready.slice(0, firstCheckIdx);
  return ready;
}
