...
export interface PressureThresholds {
  ...
  /**
   * L2: container swap usage at/above this % of its --memory-swap allowance.
   * Catches the container-scoped swap plateau: pages spill into the
   * container's own swap allowance, its RAM% flattens below every RAM
   * threshold, and HOST SwapFree stays high — invisible to the joint avail +
   * host-swap condition. No sustain window: cgroup swap usage is not a flappy
   * signal.
   */
  containerSwapUsedPct: number;
  ...
  /**
   * Post-storm calm hold: after any L2/L3 verdict, spawns stay frozen for
   * this long past the last high-pressure reading. Why: PSI avg10 (and its
   * cousins) decay in seconds; the instant the level drops below 2 every
   * paused run resumed AT ONCE, pressure re-spiked within ~15 s and the guard
   * drained the pool again — a pause↔resume thundering herd that re-paid the
   * in-flight LLM turn and killed npm installs mid-write every cycle. The
   * hold gates ADMISSION ONLY (never keeps shedding). 0 disables.
   */
  reopenCalmMs: number;
}
...
export function resolveGuardThresholds(
  ...
  return {
    ...
    containerSwapUsedPct: num(
      'HUU_GUARD_CONTAINER_SWAP_PCT',
      DEFAULT_PRESSURE_THRESHOLDS.containerSwapUsedPct,
      1,
      100,
    ),
    ...
    reopenCalmMs: num(
      ...
      DEFAULT_PRESSURE_THRESHOLDS.reopenCalmMs,
      0,
      600_000,
    ),
  };
...
export class PressureLadder {
  ...
  /** End of the post-storm calm window (extended on every L2/L3 verdict). */
  private stormCalmUntil = Number.NEGATIVE_INFINITY;
   ...
   * Also feeds the post-storm calm window (see {@link spawnHold}).
   */
  evaluate(m: SystemMetrics, budgetBytes: number, now: number = Date.now()): PressureVerdict {
    const verdict = this.grade(m, budgetBytes, now);
    if (verdict.level >= 2) this.stormCalmUntil = now + this.t.reopenCalmMs;
    return verdict;
  }

  /**
   * True while spawns must stay frozen: during an L2/L3 storm AND for
   * `reopenCalmMs` after the last high-pressure verdict. Gates ADMISSION only
   * — preemption decisions come from the live verdict, never from this hold
   * (a calm window must not keep shedding agents). Prevents the thundering
   * herd: without it, every paused run resumed the instant avg10 dipped below
   * the line and the storm re-formed within seconds.
   */
  spawnHold(now: number = Date.now()): boolean {
    return now < this.stormCalmUntil;
  }

  private grade(m: SystemMetrics, budgetBytes: number, now: number): PressureVerdict {
    const availPct = m.ramTotalBytes > 0 ? (m.ramAvailableBytes / m.ramTotalBytes) * 100 : 100;
    // Host-preferred availability for the earlyoom joint conditions: the swap
    // half of the pair is HOST-scoped, so the avail half must be able to see
    // the host too — a container can sit at 50% of its cgroup while the HOST
    // (browser/IDE outside the cgroup) is nearly out. Take the tighter of the
    // two scopes; on a native Linux host they coincide, off-Linux the host
    // pair is null and this degrades to the cgroup/host figure unchanged.
    const hostAvailPct =
      m.hostMemTotalBytes !== null && m.hostMemTotalBytes > 0 && m.hostMemAvailableBytes !== null
        ? (m.hostMemAvailableBytes / m.hostMemTotalBytes) * 100
        : null;
    const effAvailPct = hostAvailPct === null ? availPct : Math.min(availPct, hostAvailPct);
    const availScope = hostAvailPct !== null && hostAvailPct < availPct ? 'host avail' : 'avail';
    ...
    // Container-scoped swap spill (cgroup v2 memory.swap.current/max): null
    // when not containerized / unlimited / unreadable — never fires then.
    const containerSwapPct =
      m.containerSwapTotalBytes !== null &&
      m.containerSwapTotalBytes > 0 &&
      m.containerSwapUsedBytes !== null
        ? (m.containerSwapUsedBytes / m.containerSwapTotalBytes) * 100
        : null;
    const containerSwapHot =
      containerSwapPct !== null && containerSwapPct >= this.t.containerSwapUsedPct;
    ...
    if (
      (effAvailPct < this.t.availPctEmergency && swapFreePct < this.t.swapFreePctEmergency) ||
      ...
      return {
        ...
        reason:
          full10 !== null && full10 >= this.t.psiFullEmergency
            ...
            : `${availScope} ${effAvailPct.toFixed(1)}% + swap free ${swapFreePct.toFixed(1)}% below emergency floor`,
        overshootBytes,
      };
    }
    ...
    if (
      (effAvailPct < this.t.availPct && swapFreePct < this.t.swapFreePct) ||
        ...
        m.ramPercent >= this.t.destroyPercent
          ...
          : full10 !== null && full10 >= this.t.psiFullHigh
            ...
            : swapInSustained
              ...
              : containerSwapHot
                ? `container swap ${containerSwapPct!.toFixed(1)}% of allowance ≥ ${this.t.containerSwapUsedPct}%`
                : `${availScope} ${effAvailPct.toFixed(1)}% + swap free ${swapFreePct.toFixed(1)}% low`;
