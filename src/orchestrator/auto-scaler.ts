...
export class AutoScaler {
  ...
  /**
   * Smoothed host MemAvailable (bytes) feeding the host-availability clamp —
   * asymmetric in the DANGER direction (inverse of the footprint EMA): a DROP
   * applies instantly (a browser window opening on the host must bind
   * admission NOW), recovery is damped (α 0.2 per slow tick, ~63% in ~4.5 s)
   * so closing a tab doesn't instantly re-admit a burst. null when the host
   * signal is absent.
   */
  private hostAvailSmoothedBytes: number | null = null;
  private lastHostSmoothAt = 0;
  /** True when the last headroom computation was bound by the HOST clamp. */
  private lastHostClampBinding = false;
  ...
  private pollDelayMs(): number {
    const { ramUsedBytes, memPressureSome10 } = this.currentMetrics;
    const budgetBytes = this.dialBudgetBytes();
    ...
    // Same stale-window argument when the HOST is the edge: while the host
    // clamp is what's binding, sample fast too.
    return nearBudget || psiWarm || this.lastHostClampBinding
  ...
  /**
   * Degrade to sequential, never to zero.
   *
   * Every gate in {@link shouldSpawn} except COOLDOWN keys on MACHINE state —
   * RAM%, CPU%, PSI, byte headroom — and an overshoot originating OUTSIDE huu
   * (a browser/IDE holding RAM past a low dial, a loaded box) holds those true
   * INDEFINITELY. With nothing of ours live that is not "slower", it is zero
   * agents FOREVER: the pool spins, cost stays $0, the run never finishes.
   * One agent is the progress guarantee, mirroring the floor already present
   * in `AdmissionController.shouldAdmit` (liveAdmitted === 0 satisfies the byte
   * check), `GlobalScheduler.shouldSpawn` (L1 freezes only while busy > 0) and
   * `budgetCeiling()`'s `max(1, …)`.
   *
   * COOLDOWN is deliberately NOT floored: that hold is OURS, it is time-bounded
   * and self-releases (see {@link notifyAgentDestroyed}), so it can never
   * deadlock. The guard stays symmetric — `shouldDestroy()` already bails at
   * zero agents and the ladder's L1 never preempts the last survivor.
   */
  private isLastResortSpawn(): boolean {
  ...
  shouldSpawn(): boolean {
    ...
    const lastResort = this.isLastResortSpawn();
    if (this.mode === 'manual') {
      ...
      // At zero live agents there is no "someone else" — floor of one.
      return lastResort || ramPercent < this.config.destroyThresholdPercent;
    }
  ...
  /**
   * The dial budget against the CURRENT total, container-aware: when the total
   * IS a cgroup limit, the OS reserve is NOT re-subtracted — the host-side
   * wrapper already carved it out of `--memory` (see {@link ramBudgetBytes}).
   */
  private dialBudgetBytes(): number {
    const { ramTotalBytes, containerAware } = this.currentMetrics;
    return ramBudgetBytes(ramTotalBytes, this.config.budgetPercent, { containerAware });
  }

  /**
   * Live host-availability clamp: bytes huu may still claim before the MACHINE
   * (not the cgroup) runs out. Only engaged when the memory figures are
   * container-scoped — in the host-fallback branch `ramUsedBytes` already
   * includes every other host process, so the budget math self-corrects and no
   * clamp is needed. null = no clamp (not containerAware, host signal absent,
   * or HUU_NO_HOST_CLAMP).
   */
  private hostClampBytes(): number | null {
    const m = this.currentMetrics;
    if (!m.containerAware) return null;
    const avail = this.hostAvailSmoothedBytes ?? m.hostMemAvailableBytes;
    return hostHeadroomBytes(avail, m.hostMemTotalBytes);
  }

  /**
   * Byte headroom every admission decision plans with — the single choke point
   * behind budgetAdditional() and headroomBytesRemaining(), so the greedy
   * spawn gate, the PSI controller ceiling, headroomCapacity() and the
   * GlobalScheduler run-admission signal all inherit the same math:
   *
   *   budgetHeadroom = max(0, dialBudget − used − reserved)
   *   hostHeadroom   = max(0, hostClamp − reserved)      // when clamped
   *   headroom       = min(budgetHeadroom, hostHeadroom)
   *
   ...
   * reading and the budget is blown before it is ever observed. Reservations
   * charge BOTH sides: a spawn in flight is equally invisible to cgroup `used`
   * and to host MemAvailable until it materializes.
   */
  private clampedHeadroomBytes(): number {
    ...
    const budgetHeadroom = Math.max(0, this.dialBudgetBytes() - ramUsedBytes - reserved);
    const clamp = this.hostClampBytes();
    if (clamp === null) {
      this.lastHostClampBinding = false;
      return budgetHeadroom;
    }
    const hostHeadroom = Math.max(0, clamp - reserved);
    this.lastHostClampBinding = hostHeadroom < budgetHeadroom;
    return Math.min(budgetHeadroom, hostHeadroom);
  }

  /** How many MORE agents fit under min(RAM budget, host availability). */
  private budgetAdditional(): number {
    return Math.floor(this.clampedHeadroomBytes() / this.chargeBytes());
  }
  ...
  /**
   * RAM bytes still free under min(budget dial, host availability) with
   * reservations charged — the byte-denominated admission signal
   * (slots = this ÷ charge).
   */
  headroomBytesRemaining(): number {
    return this.clampedHeadroomBytes();
  }
  ...
  acceptMetrics(metrics: SystemMetrics): void {
    ...
    // The multi-run budget scaler gets its metrics PUSHED (never polled), so
    // the host-availability smoothing must run here too. Self-gated at 1 Hz.
    this.maybeSmoothHostAvail();
  }

  /**
   * Update the smoothed host-available figure (self-gated at 1 Hz regardless
   * of caller cadence): DOWN applies instantly, UP is damped — see the field
   * doc on hostAvailSmoothedBytes.
   */
  private maybeSmoothHostAvail(): void {
    const now = Date.now();
    if (now - this.lastHostSmoothAt < POLL_INTERVAL_MS) return;
    this.lastHostSmoothAt = now;
    const raw = this.currentMetrics.hostMemAvailableBytes;
    if (raw === null) {
      this.hostAvailSmoothedBytes = null;
      return;
    }
    if (this.hostAvailSmoothedBytes === null || raw < this.hostAvailSmoothedBytes) {
      this.hostAvailSmoothedBytes = raw;
      return;
    }
    this.hostAvailSmoothedBytes += DEFAULT_EMA_ALPHA * (raw - this.hostAvailSmoothedBytes);
  }
  ...
  getStatus(): AutoScaleStatus {
    ...
    // Refresh lastHostClampBinding against the current metrics — the
    // controller path of effectiveLimit() can return without recomputing it.
    this.clampedHeadroomBytes();
    const hostAvail = this.hostAvailSmoothedBytes ?? this.currentMetrics.hostMemAvailableBytes;
    return {
      ...
      targetPsi: this.config.targetPsi,
      hostAvailableMb: hostAvail === null ? null : Math.round(hostAvail / MIB),
      hostClampActive: this.lastHostClampBinding,
    };
  }
