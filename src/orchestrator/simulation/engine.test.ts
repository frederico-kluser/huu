...
describe('SimulationEngine', () => {
  it('drives the run to completion with every task terminal', () => {
    ...
    // Every agent card lands in a terminal phase — and every DONE card ends
    // MERGED (the engine mirrors the real "done means MERGED" contract, so
    // the /simulation demo shows the READY→DONE ripple, never a stuck READY).
    for (const a of state.agents) {
      expect(['done', 'no_changes', 'error']).toContain(a.phase);
      if (a.phase === 'done') {
        expect(a.merged).toBe(true);
        expect(a.mergedAt).toBeGreaterThan(0);
      } else {
        expect(a.merged).not.toBe(false); // no_changes/error never carry the flag
      }
    }
  });

  it('shows the merge ripple: done cards await the merge (merged:false) mid-integration', () => {
    const engine = makeEngine();
    let sawAwaitingMerge = false;
    runToDone(engine, (s) => {
      if (s.agents.some((a) => a.phase === 'done' && a.merged === false)) {
        sawAwaitingMerge = true;
      }
    });
    expect(sawAwaitingMerge).toBe(true);
  });
