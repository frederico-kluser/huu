...
export class SimulationEngine {
  ...
  private stepMerge(phase: MergePhase): void {
    ...
    // Per-tick merge RIPPLE — mirrors the real orchestrator's per-branch
    // onBranchMerged hook: branches land one chunk per tick (ascending order,
    // ceil(pending/ticksLeft)) and each owning card flips merged=true as its
    // branch lands. Final state is identical to the old all-at-once flip.
    const ticksLeft = Math.max(phase.ticks, 0);
    const moveCount =
      ticksLeft <= 0
        ? rec.branchesPending.length
        : Math.ceil(rec.branchesPending.length / (ticksLeft + 1));
    for (let i = 0; i < moveCount; i++) {
      const branch = rec.branchesPending.shift();
      if (!branch) break;
      rec.branchesMerged.push(branch);
      this.markBranchMerged(phase, branch);
    }
    if (phase.ticks <= 0) {
      rec.phase = 'done';
  ...
  /** Flip the card owning `branchName` to merged (the DONE-ripple mirror). */
  private markBranchMerged(phase: MergePhase, branchName: string): void {
    for (const id of phase.branchIds) {
      const card = this.cards.get(id);
      if (card && card.status.branchName === branchName) {
        card.status.merged = true;
        card.status.mergedAt = Date.now();
        return;
      }
    }
  }
  ...
  private finalizeCard(card: SimCard, phase: WorkPhase): void {
    ...
    if (card.scenario === 'error_final') {
      ...
      card.status.pushStatus = 'skipped';
      // Mirror of the real orchestrator's "done means MERGED" contract:
      // finished-but-unmerged until stepMerge lands this card's branch.
      card.status.merged = false;
      this.bump(card, 'done');
  ...
  getState(): OrchestratorState {
    ...
    return {
      ...
      // Mirror the real orchestrator's reserved-agent counter: one judge/merge
      // agent lives while the sim is judging or integrating, else zero.
      reservedAgentCount:
        this.status === 'integrating' || this.checkRuns.some((c) => c.phase === 'judging')
          ? 1
          : 0,
