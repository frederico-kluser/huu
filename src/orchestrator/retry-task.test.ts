...
describe('interactive retry (awaiting_retry → retryTask → finish)', () => {
  ...
  it(
    ...
    async () => {
      ...
      // The retry-merge path (runStageIntegration via the synthetic visit)
      // flips the retried card to merged — the reset had cleared the flags.
      expect(a.merged).toBe(true);
      expect(a.mergeFailed).toBeUndefined();
      ...
      // The REPLACED entry carries the merge truth of the final outcome.
      expect(result.manifest.agentEntries.find((e) => e.agentId === 1)!.merged).toBe(true);
    },
    20_000,
  );
  ...
  it(
    'without interactiveRetry a failed card resolves to error (M3-04: no silent done)',
    async () => {
      const pipeline: Pipeline = {
        name: 'no-hold',
        maxRetries: 0,
      ...
      // M3-04: a step with failed tasks must NOT silently report 'done'.
      // Without interactiveRetry, the run ends in 'error' so the caller
      // knows work was lost.
      expect(sawAwaitingRetry).toBe(false);
      expect(result.manifest.status).toBe('error');
      expect(result.manifest.errorReason).toBeDefined();
      const a = result.agents.find((x) => x.agentId === 1)!;
