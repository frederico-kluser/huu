...
describe('AdmissionController', () => {
  ...
  it('floor of one: admits with zero headroom when nothing is live (external RAM overshoot)', () => {
    const c = new AdmissionController({ hysteresisChecks: 3 });
    const baseline = 384 * 1024 * 1024;
    // The box is over budget because of OTHER processes (browser/IDE), not
    // huu — with zero live runs, charging the baseline deadlocked the queue.
    const starved = ctx({ liveAdmitted: 0, headroomBytes: 0, runBaselineBytes: baseline });
    expect(c.shouldAdmit(starved)).toBe(false); // streak 1 (hysteresis still applies)
    expect(c.shouldAdmit(starved)).toBe(false); // streak 2
    expect(c.shouldAdmit(starved)).toBe(true); // streak 3 → degrade-to-sequential
  });

  it('floor of one never over-admits: a live run still pays the baseline', () => {
    const c = new AdmissionController({ hysteresisChecks: 1 });
    const baseline = 384 * 1024 * 1024;
    expect(
      c.shouldAdmit(ctx({ liveAdmitted: 1, headroomBytes: 0, runBaselineBytes: baseline })),
    ).toBe(false);
  });
