...
describe('host meminfo pair + container swap (host-aware accounting)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (os.totalmem as unknown as Mock).mockReturnValue(16 * 1024 ** 3);
    (os.freemem as unknown as Mock).mockReturnValue(8 * 1024 ** 3);
  ...
  it('defaults every host/container-swap field to null when nothing is readable', () => {
    const m = getSystemMetrics();
    expect(m.hostMemTotalBytes).toBeNull();
    expect(m.hostMemAvailableBytes).toBeNull();
    expect(m.containerSwapUsedBytes).toBeNull();
    expect(m.containerSwapTotalBytes).toBeNull();
  });

  it('reports the HOST meminfo pair even when the memory figures are cgroup-scoped', () => {
    // Inside a --memory container: constrainedMemory = 4 GiB (cgroup), while
    // /proc/meminfo (NOT namespaced) shows the 16 GiB host with 6 GiB available.
    const constrainedSpy = vi
      .spyOn(process, 'constrainedMemory')
      .mockReturnValue(4 * 1024 ** 3);
    (fs.existsSync as unknown as Mock).mockImplementation(
    ...
    expect(m.containerAware).toBe(true);
    expect(m.ramTotalBytes).toBe(4 * 1024 ** 3); // cgroup scope
    expect(m.hostMemTotalBytes).toBe(16 * 1024 ** 3); // host scope
    expect(m.hostMemAvailableBytes).toBe(6 * 1024 ** 3);

    constrainedSpy.mockRestore();
  });

  it('reads container swap usage/ceiling from cgroup v2 when container-aware', () => {
    const constrainedSpy = vi
      .spyOn(process, 'constrainedMemory')
      .mockReturnValue(4 * 1024 ** 3);
    (fs.existsSync as unknown as Mock).mockImplementation(
    ...
    (fs.readFileSync as unknown as Mock).mockImplementation((path: string) => {
      if (path === '/sys/fs/cgroup/memory.swap.current') return '2147483648\n';
      if (path === '/sys/fs/cgroup/memory.swap.max') return '4294967296\n';
      throw new Error('ENOENT');
    });

    const m = getSystemMetrics();

    expect(m.containerSwapUsedBytes).toBe(2 * 1024 ** 3);
    expect(m.containerSwapTotalBytes).toBe(4 * 1024 ** 3);

    constrainedSpy.mockRestore();
  });

  it('treats memory.swap.max="max" (unlimited) as a null ceiling', () => {
    const constrainedSpy = vi
      .spyOn(process, 'constrainedMemory')
      .mockReturnValue(4 * 1024 ** 3);
    (fs.existsSync as unknown as Mock).mockImplementation(
    ...
    (fs.readFileSync as unknown as Mock).mockImplementation((path: string) => {
      if (path === '/sys/fs/cgroup/memory.swap.current') return '1048576\n';
      if (path === '/sys/fs/cgroup/memory.swap.max') return 'max\n';
      throw new Error('ENOENT');
    });

    const m = getSystemMetrics();

    expect(m.containerSwapUsedBytes).toBe(1024 ** 2);
    expect(m.containerSwapTotalBytes).toBeNull();

    constrainedSpy.mockRestore();
  });

  it('never reads container swap when the memory figures are host-scoped', () => {
    // Host branch (no cgroup limit): the root cgroup's swap files may exist
    // on a bare host, but the host meminfo pair is the honest source there.
    (fs.existsSync as unknown as Mock).mockImplementation(
      (path: string) =>
        path === '/sys/fs/cgroup/memory.swap.current' ||
        path === '/sys/fs/cgroup/memory.swap.max',
