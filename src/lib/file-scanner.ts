...
/** Returns true when a directory has at least one selected descendant file
 * but is NOT fully selected (i.e. indeterminate / partial state). For leaf
 * files this always returns false. */
export function hasPartialSelection(node: FileNode): boolean {
  if (!node.isDirectory) return false;
  if (node.selected) return false; // fully selected → not partial
  return hasAnySelectedDescendant(node);
}

function hasAnySelectedDescendant(node: FileNode): boolean {
  if (!node.children) return false;
  for (const child of node.children) {
    if (!child.isDirectory && child.selected) return true;
    if (child.isDirectory) {
      if (child.selected) return true; // all descendants selected
      if (hasAnySelectedDescendant(child)) return true;
    }
  }
  return false;
}
