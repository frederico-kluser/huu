...
describe('pipeline-io', () => {
  ...
  it('round-trips review.onBlocked and rejects unknown values', () => {
    const original: Pipeline = {
      name: 'with-onblocked',
      steps: [
        {
          ...
          prompt: 'p $file',
          files: ['a.ts'],
          review: { prompt: 'review it', maxRounds: 2, onBlocked: 'hold' },
        },
      ],
    };
    const file = join(tmp, 'onblocked.pipeline.json');
    exportPipeline(original, file);
    expect(importPipeline(file)).toEqual(original);
    ...
    expect(() =>
      parsePipelineFromJson(
        JSON.stringify({
          name: 'bad-onblocked',
          steps: [
            { name: 'S', prompt: 'p', files: [], review: { prompt: 'r', onBlocked: 'block' } },
          ],
        }),
      ),
    ).toThrow();
  ...
  it('keeps review.onBlocked absent when not set (waive stays the default)', () => {
    const original: Pipeline = {
      name: 'no-onblocked',
      steps: [{ name: 'S', prompt: 'p', files: [], review: { prompt: 'r' } }],
    };
    const file = join(tmp, 'no-onblocked.pipeline.json');
    exportPipeline(original, file);
    const restored = importPipeline(file);
    expect(restored).toEqual(original);
    const step = restored.steps[0];
    expect(step.type === 'check' ? undefined : 'onBlocked' in (step.review ?? {})).toBe(false);
  });
  ...
  describe('v2 conditional pipelines', () => {
    ...
    it('rejects check step with two default outcomes', () => {
      const json = JSON.stringify({
        name: 'bad',
        steps: [
          { name: 'a', prompt: 'p', files: [] },
          {
            ...
            condition: 'x',
            outcomes: [
              { label: 'ok', nextStepName: 'a', default: true },
              { label: 'no', nextStepName: 'a', default: true },
            ],
          },
        ],
      });
      expect(() => parsePipelineFromJson(json)).toThrow(/exactly one.*default|default.*exactly one/i);
    });
...
describe('write-set disjunction (static validation)', () => {
  it('rejects overlapping write-sets in the same wave / write-set sobreposto reprova', () => {
    const json = JSON.stringify({
      name: 'overlap',
      steps: [
        { name: 'A', prompt: 'p', files: [], writes: ['src/foo/**'], dependsOn: [] },
        { name: 'B', prompt: 'p', files: [], writes: ['src/foo/bar.ts'], dependsOn: [] },
      ],
    });
    expect(() => parsePipelineFromJson(json)).toThrow(/overlapping write-sets/);
  });

  it('accepts non-overlapping write-sets in the same wave', () => {
    const json = JSON.stringify({
      name: 'disjoint',
      steps: [
        { name: 'A', prompt: 'p', files: [], writes: ['src/foo/**'], dependsOn: [] },
        { name: 'B', prompt: 'p', files: [], writes: ['src/bar/**'], dependsOn: [] },
      ],
    });
    const result = parsePipelineFromJson(json);
  ...
  it('accepts overlapping write-sets when steps are in different waves (dependsOn)', () => {
    const json = JSON.stringify({
      name: 'sequential',
      steps: [
        { name: 'A', prompt: 'p', files: [], writes: ['src/foo/**'], dependsOn: [] },
        { name: 'B', prompt: 'p', files: [], writes: ['src/foo/**'], dependsOn: ['A'] },
      ],
    });
    const result = parsePipelineFromJson(json);
    expect(result.steps).toHaveLength(2);
  ...
  it('ignores writes when no DAG edges exist (linear pipeline)', () => {
    const json = JSON.stringify({
      name: 'linear',
      steps: [
        { name: 'A', prompt: 'p', files: [], writes: ['src/foo/**'] },
        { name: 'B', prompt: 'p', files: [], writes: ['src/foo/**'] },
      ],
    });
    const result = parsePipelineFromJson(json);
    expect(result.steps).toHaveLength(2);
  ...
  it('glob intersection: overlapping prefixes (dir vs deep)', () => {
    const json = JSON.stringify({
      name: 'dir-vs-deep',
      steps: [
        { name: 'A', prompt: 'p', files: [], writes: ['src/**'], dependsOn: [] },
        { name: 'B', prompt: 'p', files: [], writes: ['src/foo/*.ts'], dependsOn: [] },
      ],
    });
    expect(() => parsePipelineFromJson(json)).toThrow(/overlapping write-sets/);
  });

  it('glob intersection: ** globs without prefix conservatively overlap', () => {
    const json = JSON.stringify({
      name: 'star-star',
      steps: [
        { name: 'A', prompt: 'p', files: [], writes: ['**/*.ts'], dependsOn: [] },
        { name: 'B', prompt: 'p', files: [], writes: ['**/*.test.ts'], dependsOn: [] },
      ],
    });
    expect(() => parsePipelineFromJson(json)).toThrow(/overlapping write-sets/);
  });
