...
export function reportJudgeCondition(opts: {
  ...
  return (
    ...
    `4) \`git diff --name-only $baseCommit..HEAD\` shows NO modified files outside \`.huu/\` except at most one \`.gitignore\` whose diff only rewrites a \`.huu/\` exclusion into \`.huu/*\` + a negation line (report-only contract). If the command fails or produces no output (empty range), answer "rework" and state the failure reason. ` +
    `${extra} ` +
