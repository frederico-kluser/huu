...
/**
 * Kahn's algorithm, deterministic: whenever several nodes are ready, the one
 * declared FIRST wins. Two runs over the same graph give the same array, which
 * is what lets the compiler emit byte-identical pipelines.
 *
...
export function topoOrder(graph: DevGraph): { order: string[]; cycle: boolean } {
  ...
  const edges = edgesOf(graph).filter(
...
/**
 * Every node that can reach `nodeId` by following edges backwards.
 *
 * Faithful to the drawing rather than tidy: in a cyclic graph a node CAN be its
 * own ancestor, and this returns that. Callers that need "strictly upstream"
 * (the fan-out check does) must exclude the node themselves.
 */
export function ancestorsOf(graph: DevGraph, nodeId: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const edge of inboundEdges(graph, current)) {
      if (!known.has(edge.source)) continue;
...
function longestDepth(
  ...
  for (const id of order) {
    ...
    for (const edge of inboundEdges(graph, id)) {
