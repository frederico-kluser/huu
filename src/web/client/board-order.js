...
export function createBoardOrder(lanes = ['todo', 'doing', 'done']) {
  ...
  return {
    ...
    /**
     * @param {Array<{ key: string, lane: string }>} cards with lane already one of `lanes`.
     * @returns {{ movers: Set<string>, byLane: Record<string, Array<{ key: string, lane: string }>> }} each lane sorted.
     */
    place(cards) {
      ...
      /** @type {Record<string, Array<{ key: string, lane: string }>>} */
      const byLane = {};
