...
**On a VPN (WireGuard / OpenVPN / Tailscale exit-node)?** If every agent fails with `Request timed out` and you see `tokens +0in +0out` on retries, the docker bridge MTU (1500) is larger than your VPN tunnel MTU (~1420). DNS resolves and TCP connects, but the TLS ClientHello packet is fragmented and silently dropped because Path MTU Discovery is broken across most consumer VPNs. huu now detects this with an 8-second OpenRouter reachability probe at run-start and aborts loudly. Workaround: `export HUU_DOCKER_NETWORK=host` to share the host's network namespace (and its MSS-clamping). Permanent fix: edit `/etc/docker/daemon.json` with `{"mtu": 1420}` and `sudo systemctl restart docker`.
...
| `HUU_DOCKER_NETWORK` | no | Pass-through value for `docker run --network=<value>`. Most users leave this unset (docker default bridge). Set to `host` when on a VPN whose tunnel MTU is below 1500 — the bridge drops large TLS packets and every agent hangs in retry loops. See the "On a VPN?" note above. |
