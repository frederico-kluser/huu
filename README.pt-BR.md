...
`huu --web` abre uma interface no navegador que espelha a TUI 1:1 — mesma FSM, mesmo orchestrator, mesmo back-end — porém com layout click-driven, responsivo, e atualizações em tempo real via uma única conexão WebSocket. Use quando preferir dirigir o fluxo com o mouse em vez de decorar atalhos.
...
- O servidor faz bind só em `127.0.0.1` (nunca em `0.0.0.0`) — inacessível de outras máquinas.
- Toda URL carrega um token UUID por processo (`?t=…`); o token é validado em **toda** requisição HTTP e no upgrade do WebSocket. Sem token, retorna `401`.

**Stack**

O front-end (`webui/`) é um workspace Vite + React + TypeScript com Tailwind e primitivas no estilo shadcn, organizado por Atomic Design (atoms → molecules → organisms → templates → pages). A cor fuchsia/`ai` é reservada pra afordâncias dirigidas por IA (Pipeline Assistant, Smart Select, etc.), espelhando o `theme.ai` da TUI.

> Use a TUI se preferir keyboard-first; use `--web` se preferir um fluxo visual com cliques.
...
O editor de pipeline (`N` pra criar uma etapa de trabalho, `C` pra criar uma etapa de checagem, `T` pra timeouts, `M` pro picker de modelo) cuida de tudo isso sem você sair do TUI. Referência completa de teclado: [`docs/KEYBOARD.md`](docs/KEYBOARD.md).

### Etapas condicionais (roteamento julgado por LLM)

Pipelines podem incluir **check steps** — nós de decisão cujo veredito é
produzido por um agente juiz LLM com acesso a shell rodando no worktree
de integração. Um check avalia uma `condition` em linguagem natural e
roteia pra um dos `outcomes` declarados, permitindo saltos pra frente,
loops voltando a etapas anteriores ou pular adiante. O token `$runs` na
condição é substituído pelo contador 1-based de iterações daquele check,
permitindo escrever `Se $runs >= 3, aceite mesmo assim` pra limitar loops.

Pontos importantes:

- O worktree de integração **nunca dá rewind** — voltar pra uma etapa
  anterior só re-executa ela em cima do HEAD atual, acumulando commits.
- Cada check precisa de exatamente um outcome marcado como `default: true`,
  usado quando o juiz falha em produzir veredito parseável ou retorna
  uma label desconhecida.
- Limites de segurança: `Pipeline.maxNodeExecutions` (default 50) limita
  visitas totais por run; `CheckStep.maxRuns` limita revisitas de um
  check específico.
- Veja [`example.conditional.pipeline.json`](example.conditional.pipeline.json)
  pra um exemplo de gate de cobertura.
...
| `HUU_DOCKER_PASS_ENV` | não | Lista separada por espaços de nomes de env vars adicionais pra encaminhar pro container. O wrapper sempre encaminha `OPENROUTER_API_KEY`, `OPENROUTER_API_KEY_FILE`, `HUU_CHECK_PUSH`, `HUU_WORKTREE_BASE`, `HUU_HOST_HOME`, e `TERM` — use isso pra adicionar nomes customizados. |
| `HUU_HOST_HOME` | não | Setada automaticamente pelo wrapper com o home do host. Dentro do container, `getHuuHome()` lê esta variável pra que escritas em `~/.huu/` e o destino default de export `~/Downloads/` caiam no filesystem bind-mountado do host (pipelines salvos sobrevivem a `--rm`). Fica unset fora do Docker — cai pra `homedir()`. Sobrescreva só pra testes. |
...
| `~/.huu/pipeline-memory.json` | global | Pipelines salvos no editor do TUI. Bind-mountado pra dentro do Docker (mesmo arquivo dentro e fora do container). |
| `~/.huu/pipelines/` | global | Pipelines exportados em formato `.pipeline.json` individual. Sincronizado com Docker via bind mount. |
...
**Roxo (magenta) = ações de IA.** Sempre que você ver um painel ou marcador roxo (`✦`) — o **Smart Select** (tecla `S` no seletor de arquivos), o **Assistente de pipeline**, a **Análise do projeto**, os **logs dos agentes** — há um LLM sendo invocado em seu nome. Cyan é navegação/seleção neutra; verde é confirmação/sucesso; amarelo é estado intermediário ou aviso; vermelho é erro; azul é informação auxiliar não-IA (modais auxiliares, scopes não-IA).

Os tokens ficam em [`src/ui/theme.ts`](src/ui/theme.ts). Componentes que adicionam novos usos de roxo fora de contextos de IA devem escolher outra cor.
