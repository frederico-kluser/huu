...
| **2 — semanas** | 🔨 **Parcial** (2.1 + 2.2 + 2.3 ✅ entregues + adenda 2.6 ✅; 2.4/2.5 pendentes) | cgroup-pai `memory.high/max` por %, controlador senpai+AIMD com histerese, sessão pi persistente, fila SQLite WAL, zram | OOM **global impossível** via kernel; degrada/retoma em vez de morrer | Thrash invisível; oscilação; CPU do zram |
...
> fase `paused` (coluna TODO, pois a task é literalmente re-enfileirada;
> `PAUSED` âmbar, badge `⏸N`) no Ink e na web.
>
...
### 2.6 Contabilidade host-aware + honestidade dos contadores — ✅ ENTREGUE (adenda)

> **Entregue (2026-07-03)**, nascida da auditoria "done ≠ terminado / contador
> de RAM baixo": o container era CEGO aos outros processos do host (browser/IDE
> fora do cgroup) — `--memory = total − reserva` assumia a máquina inteira para
> o huu e podia empurrar o HOST pra swap/OOM global sem o cgroup bater no teto.
>
> - **Clamp de disponibilidade do host**: o sampler lê o par
>   `MemTotal`/`MemAvailable` do `/proc/meminfo` (não-namespaced = verdade do
>   HOST mesmo dentro do container) e TODA decisão de admissão planeja com
>   `min(headroom do budget, hostAvailable − reserva)` (choke point único
>   `clampedHeadroomBytes()` no AutoScaler; smoothing assimétrico — queda
>   binda instantâneo, recuperação amortecida). Escape `HUU_NO_HOST_CLAMP=1`.
> - **Ladder host-aware**: as condições conjuntas earlyoom (L2/L3) usam o
>   `min(avail do container, avail do host)`; novo braço L2 para o spill de
>   swap DO CONTAINER (cgroup v2 `memory.swap.current/max` ≥ 50% do allowance
>   de `--memory-swap` — o platô que o SwapFree do host não vê;
>   `HUU_GUARD_CONTAINER_SWAP_PCT`).
> - **Fix da reserva dupla**: com total = teto de cgroup, `ramBudgetBytes` não
>   re-subtrai a reserva que o wrapper já tirou do `--memory` (dials ≥86%
>   deixavam de entregar o que o usuário pediu).
> - **Displays honestos**: chip web mostra escopo container vs host + tag
>   `host-limited` + toast único sem-teto-de-kernel (gap B: rootless/
>   `HUU_NO_MEM_LIMIT` — a config do incidente — também avisado no serve
>   terminal e no run log via `noKernelCeilingWarning`); TUI anexa
>   `host N GB free`; `huu status` ganha linha `host:` + preview do `--memory`.
> - **Backoff anti-churn do pause** (Fase 2.3 follow-up):
>   `min(10s × 2^(pauses−1), 120s)` antes de re-spawnar um pausado
>   (`HUU_PAUSE_BACKOFF_MS`, `0` desliga) — pressão sustentada não cicla
>   pause↔resume re-pagando o turno in-flight; sem cap de pauses (matar
>   descartaria trabalho que a máquina cheia não progrediria de qualquer
>   forma). As garantias semânticas do pause/resume (sobrevivência do trabalho
>   pré-pause até o HEAD mergeado; resume não refaz o caminho fresh; último
>   checkpoint vence) saíram da spike manual e viraram regressões
>   (`makeResumeAwareFactory` + `resolvePiSessionPlan` puro).
>
> **Follow-up (mesma data, post-mortem do storm de 8 projetos):** (a) as
> mensagens de pause/kill do guard agora carregam o MOTIVO da ladder ("paused
> by memory guard — sustained swap-in …") — o RAM% do container sozinho lia
> como insanidade ("paused (RAM 9%)"); (b) **calm hold pós-storm**
> (`HUU_GUARD_REOPEN_CALM_MS`, default 10s): depois de qualquer veredicto
> L2/L3 a admissão (spawns E novos runs) fica congelada até passar a janela —
> sem isso a manada pausada retomava inteira no instante em que o avg10 caía e
> re-derrubava o pool em ~15s; (c) **jitter determinístico up-only** no
> backoff de resume (janela ×[1, 1.5), chave `runId#agentId#pauses`) — 8 runs
> pausados no mesmo segundo expiravam no mesmo segundo; (d) dedupe de
> node-warnings no process-log-bridge (janela 60s ×N sinks) +
> `events.setMaxListeners` no boot (`HUU_MAX_EVENT_LISTENERS`, default 256 —
> `EventEmitter.defaultMaxListeners` nunca governou AbortSignal/EventTarget,
> por isso o MaxListenersExceededWarning do SDK pi continuava).
>
> **Entregue (2026-07-03, mesmo lote):** a honestidade dos contadores da 2.6
> estendida a duas superfícies — os cards do kanban e o orçamento global:
>
> - ✅ **Kanban verídico (contrato de verdade dos cards):** `DONE` verde só
>   depois do branch MESCLAR na integração (`AgentStatus.merged`, tri-state);
>   card azul `READY` enquanto terminou-mas-aguarda-merge (fica em DOING); card
>   âmbar `UNMERGED` quando a integração falha ou o run acaba sem o branch
>   entrar (`mergeFailed`); `PAUSED` → coluna TODO. Módulo puro compartilhado
>   `src/lib/card-state.ts` (espelho web `src/web/client/card-state.js`).
> - ✅ **Agentes reservados contam no orçamento:** os agentes juiz/integração
>   reservados (ids `9998`/`9999`) agora CONTAM no orçamento global de RAM
>   enquanto vivem (entram em demanda/busy), mas NUNCA são vítimas de preempção
>   e seu spawn não passa por nenhum gate — então um orçamento congelado não
>   trava uma etapa.
>
> Segue para a Fase 3: A (vítima legítima — agentes in-process ainda fazem do
> huu o alvo do OOM do cgroup), C (PSI host-wide em cgroup v1), F (EMA dilui um
> agente pesado — PSS por subprocesso).
