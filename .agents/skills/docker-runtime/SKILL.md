...
| `src/lib/huu-home.ts` | `getHuuHome()` — reads `HUU_HOST_HOME`, falls back to `homedir()`. Single source of truth for `~/.huu/...` paths across pipeline-memory, pipeline-io, model recents, and the Downloads export default |
