...
- **A green CheckStep judge is a loop-breaker safety valve, NOT proof the work is correct.** Until 2026-07-01 EVERY judge silently took its `default: true` outcome with the pi backend: `check-evaluator` parsed the `{label,reason}` verdict only from `log` events, but pi streams the judge's ANSWER as `stream`/assistant deltas, so the verdict was never seen (fixed — it now also collects assistant-stream text; see working-on-orchestrator). This first looked like "the judge model is too weak" because a defaulting judge is indistinguishable from one that couldn't emit a verdict — but it defaulted on BOTH gpt-4o-mini AND claude-sonnet-4, which was the tell. Even post-fix: give judge steps a capable model, keep the `condition` mechanically shell-checkable, make the `default` outcome the SAFE/terminal path (it fires on judge failure / unknown label / `maxRuns` cap, NOT the loop), and INSPECT the diff yourself — real defects (empty `package.json` deps, a missing entry `<script>`) sailed through a defaulting judge.
