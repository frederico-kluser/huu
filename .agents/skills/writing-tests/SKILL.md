...
4. `npm run typecheck && npm test` must pass locally. CI (`scripts/gate.sh` via `.github/workflows/gate.yml`) runs the same suite on push, but only after the fact — and it runs on a bare runner, so a test that leans on YOUR git config (a global `user.email`, say) passes here and fails there.
