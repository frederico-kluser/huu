...
- Import color tokens from `src/ui/theme.ts`. `theme.ai` (magenta) is reserved for AI-driven UI (Smart Select, Pipeline Assistant, Project Recon, agent logs).
...
- Hardcode `'magenta'` outside of LLM-driven panels/markers. If a non-AI component needs purple-ish, use `theme.info` (blue) or `'cyanBright'`.
