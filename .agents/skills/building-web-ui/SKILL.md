...
- **One SSE stream** `/events` carries server→browser run snapshots (`{type:'run', run}`); browser→server actions are fetch POSTs. The server coalesces emits to ≤1 frame / 120 ms (`BROADCAST_INTERVAL_MS`) and replays the last frame to a new EventSource (so a refresh re-syncs).
