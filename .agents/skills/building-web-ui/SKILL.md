...
> Facts verified against source on 2026-06-25; `/simulation` synthetic demo surface added + verified 2026-06-26; multi-run scheduling (Map of concurrent runs, one GlobalScheduler, per-run SSE, runId-scoped actions, project selector) added + verified 2026-06-26; interactive-retry surface (`/api/run/retry` + `/finish`, `errorKind` amber badge, inner-status-driven chrome) added + promoted from `[task:timeout-error-retry]` 2026-06-29.
